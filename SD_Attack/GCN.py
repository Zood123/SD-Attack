...
def sparse_dropout(x, keep_prob, noise_shape):
    ...
    random_tensor += tf.random_uniform(noise_shape,seed=2)
    dropout_mask = tf.cast(tf.floor(random_tensor), dtype=tf.bool)
    pre_out = tf.sparse_retain(x, dropout_mask)
    return pre_out * (1./keep_prob)

class GCN:
    def __init__(self, sizes,X_obs, name="", with_relu=True, params_dict={'dropout': 0}, gpu_id=0,
                 seed=-1):
        """
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope(name) as scope:
                ...
                initial = tf.random_uniform([self.D, sizes[0]])
                self.W1 = tf.Variable(initial,'W1' )
                initial = tf.zeros(sizes[0])
                self.b1 = tf.Variable(initial,'b1')
                ...
                initial = tf.random_uniform([sizes[0], sizes[-1]])
                self.W2 = tf.Variable(initial,'W2')
                initial =tf.zeros(sizes[-1])
                self.b2 = tf.Variable(initial,'b2')
    ...
    def train(self, An, split_train, split_val, Z_obs, patience=60, n_iters=200, print_info=True, load_model = None):


        if load_model != None:
            with open(load_model, 'rb') as handle:
                var_dump_best = pickle.load(handle)
        ...
        feed = {self.An: tf.SparseTensorValue(np.array(An.nonzero()).T, An[An.nonzero()].A1, An.shape), self.node_ids: split_train, self.node_labels: Z_obs[split_train]}
        if hasattr(self, 'training'):
            feed[self.training] = True
        for it in range(n_iters):
            _loss, _ = self.session.run([self.loss, self.train_op], feed)
            print ( 'The loss is {0} in {1} iteration'.format ( _loss, it ) )
            acc_val = eval_class_acc(split_val, self, np.argmax(Z_obs, 1), An)
            perf_val = acc_val
            print ( 'The validation accuracy performance is: {0} in {1} iteration'.format ( perf_val, it ) )

            if perf_val > best_performance:
                best_performance = perf_val
                patience = early_stopping
        ...
        with open('GCN_new.pickle', 'wb') as handle:
            pickle.dump(var_dump_best, handle)
        self.set_variables(var_dump_best)

    def test(self, split_test, Z_obs, adj):

        acc_test = eval_class_acc( split_test, self, np.argmax ( Z_obs, 1 ), adj )
        perf_test = acc_test
        print ( 'The test accuracy performance is: {0}'.format ( perf_test) )
        return perf_test


def eval_class_acc(ids_to_eval, model, z_obs, An):
    test_pred = model.predictions.eval(session=model.session, feed_dict={model.An: tf.SparseTensorValue(np.array(An.nonzero()).T, An[An.nonzero()].A1, An.shape), model.node_ids: ids_to_eval}).argmax(1)
