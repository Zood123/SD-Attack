import numpy as np
from SD_Attack import utils
from SD_Attack.w1_attack import cal_scores_D
import random


class W1_attack:
    def __init__(self, adj, z_obs, u, eig_vals, eig_vec, perturb_logs,degrees,eig_vec2,inverse_u = None,A_I=None):
        # Adjacency matrix
        self.A_I = A_I
        ...
        self.eig_vec2 = eig_vec2
        self.inverse_u = inverse_u

    def attack_model(self, delta_cutoff=0.004):
        ...
        potential_neighbours = np.column_stack((np.tile(self.u, self.N - 1), np.setdiff1d(np.arange(self.N), self.u)))
        neighbours = []
        for node in potential_neighbours:
            if self.adj[tuple(node)] == 1:
                neighbours.append(node[1])
        potential_edges = []

        for neighbour in neighbours:
            potential_edges.append(np.column_stack(
                (np.tile(neighbour, self.N - 2), np.setdiff1d(np.arange(self.N), [neighbour, self.u]))))

        if len(potential_edges) > 1:
            self.potential_edges = np.concatenate(potential_edges, 0)
        else:
            self.potential_edges = potential_edges[0]

        self.potential_edges = np.concatenate([self.potential_edges, np.column_stack(
            (np.tile(self.u, self.N - 1), np.setdiff1d(np.arange(self.N), self.u)))])

        # sample edges if the algorithm takes too much time

        presample_id = random.sample(range(0, len(self.potential_edges)), 2000)
        self.potential_edges = self.potential_edges[presample_id]
        ...
        # calculate the scores for candidates
        struct_scores = cal_scores_D(self.A_I, self.eig_vals, self.eig_vec, filtered_edges_final, self.u, self.eig_vec2,
                                     self.inverse_u, self.degrees)

        best_edge_ix = (np.array(struct_scores)).argsort()
        best_edge_ix = best_edge_ix[-(self.degrees[self.u]):]

        with open(self.save_perturb_logs, "a+") as f:
            f.write(str(self.u))

        for index in best_edge_ix:
            best_edge = filtered_edges_final[index]
            self.adj[tuple(best_edge)] = self.adj[tuple(best_edge[::-1])] = 1 - self.adj[tuple(best_edge)]

            with open(self.save_perturb_logs, "a+") as f:
                f.write(' ' + str(best_edge))
